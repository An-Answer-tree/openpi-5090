...
| H9-scale-b | 4 卡 BS64 能否提高 ACPD-v2 吞吐 | layer 10；FSDP4，physical global BS64，无梯度累积；30K | 运行中 | 5K checkpoint 为 `23.15%`；30K 尚无结论；因样本预算与 H9 不同，不作受控 batch 效果结论。 |
...
| H13 | H9 是否需要显式 contribution 注入 | 保留 layer 10 contribution loss 和 ACL，只关闭训练与推理注入；FSDP4，physical global BS64，无梯度累积；5K | 运行中 | 训练 job `130599`；验证 job `130600` 依赖训练完成；尚无结论。 |
...
| H9-scale-b | 最终 hidden 注入；FSDP4，physical global BS64，无梯度累积，30K | 129728/130703/130704 | 原任务运行至约 25.4K；完整 `24999` 已保护，`29999` 完成后自动恢复二者到同一运行目录；20K 同配置轨迹恢复任务排队；4999 历史验证为 `463/2,000`、`23.15%`；30K 尚无结论 |
...
| H11 同层注入，BS64 | 14.60% | 40.60% | 27.00% | 2.60% | 21.20% |
...
| H7.1 协议 | `experiments/acpd-exact-attention-layer-scan/protocol.md` |
| H7.1 分析 | `experiments/acpd-exact-attention-layer-scan/analysis.md` |
...
| H9-scale-b checkpoint 恢复协议 | `experiments/acpd-v2-h9-batch-scaling-30k/protocol.md`；20K 重跑 job `130704`，25K 恢复 job `130703` |
...
| H9-scale-b，4 卡 BS64 5K | 原 checkpoint 未保留；`23.15%` 验证结果完整保留 |
| H9-scale-b，4 卡 BS64 25K 保护副本 | `/opt/liutong/openpi_checkpoints/fixed_dataset/distillation/acpd_v2/batch_scaling_30k_preserved/pi05_libero_backview_acpd_v2_layer10/pi05_libero_backview_acpd_v2_lora_fsdp4_bs64_30k/24999` |
...
| H9-scale-b 轨迹恢复，5K/10K/15K/20K（预期） | `/opt/liutong/openpi_checkpoints/fixed_dataset/distillation/acpd_v2/trajectory_recovery_20k/pi05_libero_backview_acpd_v2_layer10/pi05_libero_backview_acpd_v2_lora_fsdp4_bs64_20k_keep_all/{4999,9999,14999,19999}` |
