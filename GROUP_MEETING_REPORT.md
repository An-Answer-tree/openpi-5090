...
两个方法使用相同任务和初始状态进行验证。统计时，在每个任务的 50 组配对 episode 中重复抽样 10,000 次，每次重新计算两个方法的成功率差，中间 95% 的结果构成置信区间。如果整个区间都大于 0，说明更换抽到的 episode 后仍然稳定领先，提升不太可能只是验证样本的偶然性；如果区间跨过 0，说明有些抽样结果更好、有些更差，当前数据不足以确定哪个方法更好。
