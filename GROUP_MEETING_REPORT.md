...
| 项目 | 方法 1：最终层后注入 | 方法 2：layer 10 内注入 |
|---|---|---|
...
两种方法的 teacher target、predictor 结构、零初始化 gate、stop-gradient、FSDP4、physical BS64、seed、学习率和训练预算保持一致。方法 2 与注入位置配套，改用 layer 10 attention 后的同层状态作为 predictor 输入。目前已完成实验设计，尚无实验结果。
