...
中文名：ACPD-v2 非零权重衰减。父实验：H9-Fixed（ACPD-v2 固定权重）。
主对照为同源H9-Fixed recovery 20K，次对照为SFT BS64 20K。

提交时间：2026-09-24 11:03 CST。14:18 CST复核时尚无训练或验证结论。
...
| 15K中间点全量验证 | 136119_[0-3] | 已提交，等待资源 | 每子任务1 GPU、8 CPU、24G，8小时；每任务50回合，四套共2000回合 |
| 15K结果汇总 | 136120 | 等待136119成功 | 1 CPU、2G；afterok:136119 |
| 15K配对分析 | 136123 | 等待136120成功 | 1 CPU、2G；比较 H17 与 H9、SFT |
...
训练日志显示权重调度按协议生效：10K 为 `0.2`，12.5K 为 `0.1273`，15K
达到 `0.05`。对应 raw contribution loss 为 `0.4416→0.3050`，加权项为
`0.0883→0.0153`；supervised loss 为 `0.0394→0.0237`，student task loss
为 `0.1789→0.1076`，contribution cosine 为 `0.6906→0.7683`。

总 loss 从 `0.1565` 降至 `0.0565`，其中一部分由权重下降机械造成，因此在
15K 评测完成前不能据此判断 H17 有效。
