# H9-Fixed（ACPD-v2 固定权重）分析

正式BS64实验旧名为H9-scale-b，contribution=0.2、ACL=0.5。
H17-Decay是从其恢复轨迹10K处分出的权重消融，结果单独记录；下表BS32仅为前期吞吐参考。
...
| 10K | 23.25% | 37.45% | +14.20 点 |
| 25K | 55.80% | 55.75% | -0.05 点 |
| 30K | 60.45% | 58.00% | -2.45 点 |

ACPD-v2 从 5K 到 30K 的绝对成功率提高 `34.85` 点，但相对匹配 SFT 的优势从
`+9.30` 点变为 `-2.45` 点。10K仍有明显优势，15K验证部分完成，20K验证等待训练；
在中间点完成前，不能确定10K至25K之间优势消失的具体step。
...
cosine 只增加 `0.0054`，gate 同时下降 `11.25%`。这些趋势不能确定35K成功率提高的
原因；需要匹配SFT 35K结果判断额外训练后的相对优势，及单变量消融检验机制。
...
这不支持“后期组合辅助梯度频繁反向破坏 flow”的解释，但没有排除容量和轨迹约束：强度接近
...
- flow loss 不能经注入支路直接调整 predictor；共享表征变化仍可间接改变预测向量；
- 在注入支路中，一个全局标量 gate 根据任务 loss 调整注入强度。

H13 在该5K设置下支持注入带来 `+2.55` 点。Gate在约12K后下降，但其大小不能单独
确定实际注入量或作用。缺少直接任务梯度是否限制后期注入收益，仍需独立消融验证。
...
已提交的 H17-Decay 从完整 10K 状态继续到 20K：保持 ACL `0.5` 和非零 contribution
